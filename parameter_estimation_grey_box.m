...
parameters2 = {'d_m',d_m_I;'I_m',I_m_I;'m_c',m_c_I};
...
function [A,B,C,D] = sys_matrices2(d_m,I_m,m_c,Ts) %Ts is needed for greyest
    A = [(0.024*0.085^2+I_m)/(0.024*0.08101^2)*-1.731e-05/I_m, -0.085/0.08101*(-d_m/I_m), (-(9.812*(0.024*0.085^2 + I_m))/(I_m*0.08101)), 0; 
         -0.085/0.08101*-1.731e-05/I_m, (-d_m/I_m),    (9.812*0.024*0.085)/I_m, 0;
         eye(2)      zeros(2)];
    B = [-0.085/0.08101*m_c/I_m; 
         m_c/I_m;
         zeros(2,1)];
    C = [zeros(2) eye(2)];
    D = [0;0];
