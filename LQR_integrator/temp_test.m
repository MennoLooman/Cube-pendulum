...
reference = [zeros(2,1/h),ref_magn*ones(2,5/h),-ref_magn*ones(2,5/h),zeros(2,4/h)];
...
%output
    y(:,i) = Cd*x_nonlin(:,i) - reference(:,i) - offset(:,i); 
%state estimation
    x_hat(:,i+1) = (Ad - Kd*Cd)*x_hat(:,i) + Bd*u(i) + Kd*y(:,i);
%integral action
    int_act(i+1) = int_act(i) + (x_hat(3,i) - 0)*h;
end
